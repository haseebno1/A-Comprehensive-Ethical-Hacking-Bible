...
# Platform-specific for Kali Linux
...
def get_arguments():
    ...
    parser = argparse.ArgumentParser(description='MAC Address Changer for Kali Linux')
...
def list_interfaces():
    ...
    try:
        # Get network interfaces using ip link show
        ...
        if interfaces:
            for interface in interfaces:
                ...
                mac_result = subprocess.run(["ip", "link", "show", interface], 
                                          capture_output=True, text=True, check=True)
                mac_search = re.search(r"link/ether ([0-9a-f:]{17})", mac_result.stdout)
                
                if mac_search:
                    mac = mac_search.group(1)
                    print(f"{Colors.GREEN}Interface: {Colors.BOLD}{interface}{Colors.ENDC}")
                    print(f"  Current MAC: {mac}")
                    
                    # Get interface state
                    state_search = re.search(r"state (\w+)", mac_result.stdout)
                    if state_search:
                        state = state_search.group(1)
                        print(f"  State: {state}")
        else:
            print(f"{Colors.WARNING}No interfaces found.{Colors.ENDC}")
...
def get_current_mac(interface):
    ...
    try:
        # Use ip link to get the current MAC address
        ip_result = subprocess.run(["ip", "link", "show", interface], 
                                  capture_output=True, text=True, check=True)
        
        # Extract MAC address using regex
        mac_address_search = re.search(r"link/ether ([0-9a-f:]{17})", ip_result.stdout)
...
def main():
    ...
    # Print banner
    print(f"{Colors.HEADER}{Colors.BOLD}MAC Address Changer for Kali Linux{Colors.ENDC}")
